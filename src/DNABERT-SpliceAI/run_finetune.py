    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice":
                ...
                probs = softmax(torch.tensor(preds, dtype=torch.float32)).numpy()
            elif args.task_name == 'deepsea' or args.task_name == 'spliceai':
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        if args.output_mode == "classification":
            if args.task_name[:3] == "dna" and args.task_name != "dnasplice":
                ...
                # TODO: выход модели сразу в сигмоиду
                probs = torch.sigmoid(torch.tensor(preds, dtype=torch.float32)).numpy()
            preds = np.argmax(preds, axis=1)
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if args.n_process == 1:
            ...
            for i in range(n_proc):
                results.append(p.apply_async(convert_examples_to_features, args=(examples[indexes[i]:indexes[i+1]], tokenizer, max_length, task, label_list, output_mode, pad_on_left, pad_token, pad_token_segment_id, True,  )))
...
#     print(args.n_gpu, args.device, sep = '\t')
#     sys.exit()
